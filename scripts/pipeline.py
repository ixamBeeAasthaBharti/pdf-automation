...
#   python scripts/pipeline.py --fetch --count 20       # Download next 20 PDFs
#   python scripts/pipeline.py --fetch --ids 1474,1492  # Fetch specific IDs
#   python scripts/pipeline.py --fetch --from-file      # Fetch IDs listed in ids.txt
...
#     parser.add_argument("--from-file", action="store_true", dest="from_file",
#                         help="Read IDs from ids.txt in project root (one ID per line)")
...
#                 print(f"[Pipeline] ids.txt not found at {ids_file}")
#                 print("[Pipeline] Create it with one ID per line.")
#                 raise SystemExit(1)
#             lines = ids_file.read_text(encoding="utf-8").splitlines()
#             target_ids = [int(ln.strip()) for ln in lines
#                           if ln.strip() and not ln.strip().startswith("#") and ln.strip().isdigit()]
#             print(f"[Pipeline] Loaded {len(target_ids)} ID(s) from ids.txt")
#         elif args.ids:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  python scripts/pipeline.py --fetch                  # Download 1 PDF from MySQL
  python scripts/pipeline.py --fetch --count 20       # Download next 20 PDFs
  python scripts/pipeline.py --process                # Convert all HTML-ready queue items
  python scripts/pipeline.py --process --count 10     # Convert only the first 10 items
  python scripts/pipeline.py --status                 # Show job status table
        """,
    )
    parser.add_argument("--fetch",   action="store_true", help="Fetch next PDF(s) from MySQL")
    parser.add_argument("--count",   type=int, default=None, help="Number of PDFs to fetch/process")
    parser.add_argument("--ids",     type=str, help="Comma-separated specific MySQL IDs to fetch (e.g. 1474,7908)")
    parser.add_argument("--process", action="store_true", help="Process HTML-ready queue items")
    parser.add_argument("--status",  action="store_true", help="Show current job statuses")
    args = parser.parse_args()
    ...
    if args.fetch:
        ...
        if args.ids:
            target_ids = [int(x.strip()) for x in args.ids.split(",") if x.strip().isdigit()]
        do_fetch(count=args.count or 1, target_ids=target_ids)
    elif args.process:
        do_process()
    elif args.status:
        do_status()
